...
export default function CheckoutInvitado() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!data?.id || !data?.order_number || !data?.access_code) {
        throw new Error(
          "Supabase no devolvió la información completa del pedido invitado."
        );
      }

      const confirmedProducts = (data.items ?? cart).map((item) => ({
      ...
      const order = {
        ...
        phone: data.customer_phone ?? customerPhone,
  ...
  return (
    <main style={pageStyle}>
      <Link to="/continuar-compra">← Volver a opciones de compra</Link>

      <h1>Comprar como invitado</h1>

      <p>Completa tus datos para registrar el pedido sin crear una cuenta.</p>
      ...
      <form onSubmit={handleSubmit} style={formStyle}>
        ...
        <div style={fieldStyle}>
          ...
          <small>
            Este correo se utilizará para consultar tu pedido y recibir
        ...
        <section style={summaryStyle}>
          ...
          <strong>Total estimado: ₡{total.toLocaleString("es-CR")}</strong>
        ...
        >
          {isProcessing ? "Guardando pedido..." : "Confirmar pedido como invitado"}
        </button>
      </form>
    </main>
  );
